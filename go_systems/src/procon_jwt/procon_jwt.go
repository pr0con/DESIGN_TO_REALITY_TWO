...
func GenerateJWT(privkeyfile *rsa.PrivateKey) (string, error) {
	...
	in30m := time.Now().Add(time.Duration(30) * time.Minute).Unix()
	
	token.Claims = jwtgo.MapClaims{
        "iss":    "void.pr0con.com", // who creates the token and signs it
        "aud":    "void.pr0con.com", // to whom the token is intended to be sent
        "exp":    in30m,             // time when the token will expire (10 minutes from now)
        ...
        "scopes": "api:full",        // token scope - not a standard claim			
	}
